...
public final class OneDimAveragingPhaser {
    ...
    public static void runParallelFuzzyBarrier(final int iterations,
            ...
            final int tasks) {
        // Phaser ph = new Phaser(0);
        // // for (int ii = 0; ii < tasks; ii++) ph[ii] = new Phaser(ii);
        // ph.bulkRegister(tasks);
        ...
        for (int ii = 0; ii < tasks; ii++) {
            ...
            threads[ii] = new Thread(() -> {
                ...
                for (int iter = 0; iter < iterations; iter++) {
                    ...
                    // signal whoever need them that they are ready
                    // int currentPhase = ph[i + 1].arrive();
                    ph[i + 1].arrive();
                    ...
                    if (i > 0) ph[i].awaitAdvance(iter);
                    if (i < tasks - 1) ph[i + 2].awaitAdvance(iter);
